...
byte EMPTY_BRIGHT = 20;           // яркость "не горящих" светодиодов (0 - 255)
...
#define LOW_COLOR HUE_AQUA        // цвет низких частот
#define MID_COLOR HUE_PINK      // цвет средних
#define HIGH_COLOR HUE_PINK       // цвет высоких
...
#define STROBE_COLOR HUE_PINK     // цвет стробоскопа
...
// ------------------------------ ДЛЯ РАЗРАБОТЧИКОВ --------------------------------
#define MODE_AMOUNT 11      // количество режимов
...
DEFINE_GRADIENT_PALETTE(soundlevel_gp) {
  0,    64,   224,  208, // бирюзовый (approximation)
  128,  128,  0,    128, // фиолетовый (approximation)
  255,  255,  105,  180,  // розовый (approximation)
  200,  255,  50,   0,  // red
  255,  255,  0,    0   // red
};
...
void mainLoop() {
  ...
  if (ONstate) {
    if (millis() - main_timer > MAIN_LOOP) {
      ...
      if (this_mode == 12 || this_mode == 13 || this_mode == 1 || this_mode == 7 || this_mode == 8|| this_mode == 10) {
        analyzeAudio();
        colorMusic[0] = 0;
        colorMusic[1] = 0;
        colorMusic[2] = 0;
      ...
      if (this_mode == 3 || this_mode == 5 || this_mode == 6 || this_mode == 4 || this_mode == 9) animation();

      if (!IRLremote.receiving())    // если на ИК приёмник не приходит сигнал (без этого НЕ РАБОТАЕТ!)
        FastLED.show();         // отправить значения на ленту

      if (this_mode != 8 & this_mode != 7)       // 7 режиму не нужна очистка!!!
...
void animation() {
  // согласно режиму
  switch (this_mode) {
    /*case 0: //Середина белая, розовое по краям, фигня статично
      count = 0;
    ...
    case 0: //Радуга, оставляю
      ...
      break;
    /*case 2: // 3 цвета с середины, фигня
      for (int i = 0; i < NUM_LEDS; i++) {
    ...
    /*case 3: // 3 цвета от края, фигня
      ...
      break;*/
    case 1:  //Разноцветный страбоскоп
    ...
    case 7: // Беглый, лучший!
      ...
      break;
    /*case 8: //Дуракций второй средний
      ...
      break;*/
      case 5: // Чисто радуга без пляса
      switch (light_mode) {
        case 2: for (int i = 0; i < NUM_LEDS; i++) leds[i] = CHSV(LIGHT_COLOR, LIGHT_SAT, 255);
          break;
        case 1:
          if (millis() - color_timer > COLOR_SPEED) {
            color_timer = millis();
      ...
      case 6: // Чисто розовый 
        switch (light_mode) {
          case 0: for (int i = 0; i < NUM_LEDS; i++) leds[i] = CHSV(STROBE_COLOR, LIGHT_SAT, 255);
            break;
      }
      break;
      ...
      case 4: // Чисто белый
        switch (light_mode) {
          case 0: for (int i = 0; i < NUM_LEDS; i++) leds[i] = CHSV(100, LIGHT_SAT, 255);
            break;
      }
      break;

      case 8: // Беглый, красный
      switch (freq_strobe_mode) {
        case 0:
          if (running_flag[2]) leds[NUM_LEDS / 2] = CHSV(HUE_RED, 255, thisBright[2]);
          else if (running_flag[1]) leds[NUM_LEDS / 2] = CHSV(HUE_RED, 255, thisBright[1]);
          else if (running_flag[0]) leds[NUM_LEDS / 2] = CHSV(HUE_RED, 255, thisBright[0]);
          else leds[NUM_LEDS / 2] = CHSV(EMPTY_COLOR, 255, EMPTY_BRIGHT);
          break;
        case 1:
          if (running_flag[2]) leds[NUM_LEDS / 2] = CHSV(HIGH_COLOR, 255, thisBright[2]);
    ...
    case 9: // режим горящего огня
      for (int i = 0; i < NUM_LEDS; i++) {
        // Генерация случайного цвета в пределах оттенков огня
        int hue = random(0, 35); // огонь обычно имеет оттенки от красного до желтого
        int brightness = random(150, 255); // переменная яркость для создания эффекта мерцания
        leds[i] = CHSV(hue, 255, brightness);
      }
      delay(80); // небольшая задержка для создания эффекта мерцания огня
      break;

    case 10:  // новый режим на основе 7 цветов
      int colorIndex;
      // Определение активной частоты
      uint8_t activeFreq = 0;
      if (running_flag[2]) activeFreq = freq_max;
      else if (running_flag[1]) activeFreq = freq_max;
      else if (running_flag[0]) activeFreq = freq_max;

      // Масштабирование активной частоты к индексу цвета
      colorIndex = map(activeFreq, 0, 255, 0, 6);

      uint8_t hue = freqColors[colorIndex];
      for (int i = 0; i < NUM_LEDS; i++) {
          leds[i] = CHSV(hue, 255, thisBright[colorIndex]);
      }
